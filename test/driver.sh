...
# -MP
$cxx -MF $tmp/mp -MP -M -I$tmp $tmp/out.c
...
# -MT
$cxx -MT foo -M -I$tmp $tmp/out.c | grep -q '^foo:'
check -MT
$cxx -MT foo -MT bar -M -I$tmp $tmp/out.c | grep -q '^foo bar:'
check -MT

# -MD
echo '#include "out2.h"' > $tmp/md2.c
echo '#include "out3.h"' > $tmp/md3.c
(cd $tmp; $OLDPWD/$cxx -c -MD -I. md2.c md3.c)
grep -q -z '^md2.o:.* md2\.c .* ./out2\.h' $tmp/md2.d
check -MD
grep -q -z '^md3.o:.* md3\.c .* ./out3\.h' $tmp/md3.d
check -MD

$cxx -c -MD -MF $tmp/md-mf.d -I. $tmp/md2.c
grep -q -z '^md2.o:.*md2\.c .*/out2\.h' $tmp/md-mf.d
check -MD

# -MQ
$cxx -MQ foo -M -I$tmp $tmp/out.c | grep -q '^foo:'
check -MQ
...
# -L
echo 'extern int bar; int foo() { return bar; }' > $tmp/foo.c
$cxx -fPIC -shared -o $tmp/libfoobar.so $tmp/foo.c
echo 'int foo(); int bar=3; int main() { foo(); }' > $tmp/bar.c
$cxx -o $tmp/foo $tmp/bar.c -L$tmp -lfoobar
check -L
